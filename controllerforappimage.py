...
parser = argparse.ArgumentParser(description="Console Stream Controller Bridge")
parser.add_argument('--console', choices=['switch', 'ps4', 'ps5'], default='switch',
                    help='Console to emulate: switch, ps4, or ps5 (default: switch)')
parser.add_argument('--pico-ip', type=str, default='192.168.1.100',
                    help='Pico IP address (default: 192.168.1.100)')
    ...
    for event in events:
        ...
        if event.ev_type == "Absolute":
            ...
            if event.code == "ABS_RY":
                ...
                send_needed = True
            if event.code == "ABS_Z":
                axes_state[4] = normalize_axis(event.state)
                send_needed = True
            if event.code == "ABS_RZ":
                axes_state[5] = normalize_axis(event.state)
                send_needed = True
    if send_needed:
        send_state()
